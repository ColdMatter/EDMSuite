...
namespace CaFBECHardwareController.Controls
{
    public partial class SourceTabView : CaFBECHardwareController.Controls.GenericView
    {
        ...
        public SourceTabView(SourceTabController controllerInstance) : base(controllerInstance)
        {
            ...
            GraphPMT.Series.Add("PMT").ChartType = SeriesChartType.FastLine;
            GraphPMT.Series["PMT"].XValueType = ChartValueType.Double;

            GraphAbs.Series.Add("Abs").ChartType = SeriesChartType.FastLine;
            GraphAbs.Series["Abs"].XValueType = ChartValueType.Double;
        }
        ...
        {
            UpdateRenderedObject<Chart>(GraphPMT, (Chart obj) => { obj.Series["PMT"].Points.Clear(); });
            DataPoint[] points = new DataPoint[x.Length];
            UpdateRenderedObject<Chart>(GraphPMT, (Chart obj) => { obj.Visible = false; });
            for (int i=0; i<x.Length; i++)
            {
                UpdateRenderedObject<Chart>(GraphPMT, (Chart obj) => { obj.Series["PMT"].Points.AddXY(x[i], y[i]); });
            }
            UpdateRenderedObject<Chart>(GraphPMT, (Chart obj) => { obj.ChartAreas[0].AxisX.Minimum = Math.Floor(x.Min()); });
            UpdateRenderedObject<Chart>(GraphPMT, (Chart obj) => { obj.ChartAreas[0].AxisX.Maximum = Math.Ceiling(x.Max()); });
            UpdateRenderedObject<Chart>(GraphPMT, (Chart obj) => { obj.ChartAreas[0].AxisX.Interval = 1.0; });
            UpdateRenderedObject<Chart>(GraphPMT, (Chart obj) => { obj.ChartAreas[0].AxisX.LabelStyle.Format = "0"; });
            UpdateRenderedObject<Chart>(GraphPMT, (Chart obj) => { obj.Visible = true; });
        }

        public void UpdateGraphAbs(double[] x, double[] y)
        {
            UpdateRenderedObject<Chart>(GraphAbs, (Chart obj) => { obj.Series["Abs"].Points.Clear(); });
            DataPoint[] points = new DataPoint[x.Length];
            UpdateRenderedObject<Chart>(GraphAbs, (Chart obj) => { obj.Visible = false; });
            for (int i = 0; i < x.Length; i++)
            {
                UpdateRenderedObject<Chart>(GraphAbs, (Chart obj) => { obj.Series["Abs"].Points.AddXY(x[i], y[i]); });
            }
            UpdateRenderedObject<Chart>(GraphAbs, (Chart obj) => { obj.ChartAreas[0].AxisX.Minimum = Math.Floor(x.Min()); });
            UpdateRenderedObject<Chart>(GraphAbs, (Chart obj) => { obj.ChartAreas[0].AxisX.Maximum = Math.Ceiling(x.Max()); });
            UpdateRenderedObject<Chart>(GraphAbs, (Chart obj) => { obj.ChartAreas[0].AxisX.Interval = 1.0; });
            UpdateRenderedObject<Chart>(GraphAbs, (Chart obj) => { obj.ChartAreas[0].AxisX.LabelStyle.Format = "0"; });
            UpdateRenderedObject<Chart>(GraphAbs, (Chart obj) => { obj.Visible = true; });
        }
