...
public class Patterns : MOTMasterScript
{
    public Patterns()
    {
        ...
        Parameters["Frame0TriggerDuration"] = 10000;
        ...
        */
        ...
        // magtrap //
        Parameters["MagtrapDuration"] = 1000;
        ...
        */
        ...
        Parameters["FreeExpTime"] = 6000;

        Parameters["TransportEdgeDur"] = 260000;

        // END OF PATTERN //

        Parameters["MOTCoilsSwitchOff"] = 40000;

        Parameters["MoveTime"] = 160000; //DONT CHANGE- THIS IS THE TIME IT TAKES FROM TRANSPORT TRACK MOVE TRIG TO TWEEZER CHAMBER ARRIVAL
        //Parameters["ImageDelay"] = 340000;

        Parameters["LoadDur"] = 5000;
    ...
    public override Dictionary<string, List<List<double>>> GetDDSPattern()
    {
        ...
        int HandoverTweezerStart = MoveStart + (int)Parameters["MoveTime"];
        int LoadStart = HandoverTweezerStart + (int)Parameters["HandoverDur"];
        int LoadEnd = LoadStart + (int)Parameters["LoadDur"];
        int Image = LoadEnd + (int)Parameters["FreeExpTime"];
        int AllCoilsOff = Image + (int)Parameters["Frame0TriggerDuration"]+1000;
        ...
        addDDSPattern(p, "LoadTweezer", LoadStart,
            (double)Parameters["SF"], (double)Parameters["ResonanceDDS2"], (double)Parameters["ResonanceDDS3"], (double)Parameters["ResonanceDDS4"],
            (double)Parameters["MOTAmpDDS1"], (double)Parameters["LightoffDDS1"], (double)Parameters["LightoffDDS1"], (double)Parameters["LightoffDDS1"]);

        addDDSPattern(p, "FreeFall", LoadEnd,
            (double)Parameters["SF"], (double)Parameters["ResonanceDDS2"], (double)Parameters["ResonanceDDS3"], (double)Parameters["ResonanceDDS4"],
            (double)Parameters["LightoffDDS1"], (double)Parameters["LightoffDDS1"], (double)Parameters["LightoffDDS1"], (double)Parameters["LightoffDDS1"]);
    ...
    public override PatternBuilder32 GetDigitalPattern()
    {
        ...
        //p.Pulse(0, Image, (int)Parameters["Frame0TriggerDuration"], "cameraTrigger");  //camera trigger for MOT recap 
        ...
        p.DownPulse(0, MoveStart, (int)Parameters["TransportEdgeDur"], "transportTrack");

        p.Pulse(0, Image, (int)Parameters["Frame0TriggerDuration"], "rbAbsImgCamTrig");
    ...
    public override AnalogPatternBuilder GetAnalogPattern()
    {
        ...
        */
        ...
        p.AddLinearRamp("transferCoils", HandoverTweezerStart, (int)Parameters["HandoverDur"], 0.0);
        p.AddLinearRamp("TweezerCoils", HandoverTweezerStart, (int)Parameters["HandoverDur"], 0.48);
        p.AddAnalogValue("TweezerCoils", LoadStart, 0.0);

        //p.AddAnalogValue("TweezerCoils", AllCoilsOff, 0.0);
        //p.AddAnalogValue("MOTCoilsCurrent", Image + (int)Parameters["Frame0TriggerDuration"] + 500, 0.0);
