...
public class Patterns : MOTMasterScript
{
    public Patterns()
    {
        ...
        //Lambda Cooling
        Parameters["LambdaGlobalDetuning"] = 40.0;
        Parameters["LambdaTwoPhotonDetuning"] = 0.0;

        Parameters["SidebandFreq1Lambda"] = (double)Parameters["SidebandFreq1"] - 
            (double)Parameters["LambdaGlobalDetuning"] / 2.0;
        Parameters["SidebandFreq2Lambda"] = (double)Parameters["SidebandFreq2"] - 
            ((double)Parameters["LambdaGlobalDetuning"] + (double)Parameters["LambdaTwoPhotonDetuning"]) / 2.0;
        ...
        Parameters["FeedforwardFactorS"] = -0.12;
        Parameters["FeedforwardFactorF"] = -0.08;

        //Time
        Parameters["motLoadTime"] = 5000;
        Parameters["LambdaCoolingDuration"] = 1500;
    ...
    public override PatternBuilder32 GetDigitalPattern()
    {
        ...
        int lambdaCoolingEndTime = motLoadEndTime + (int)Parameters["LambdaCoolingDuration"];
        int cameraTrigger = lambdaCoolingEndTime + (int)Parameters["FreeExpansionTime"];


        MOTMasterScriptSnippet lm = new LoadMoleculeMOT(p, Parameters);  // This is how you load "preset" patterns.


        p.Pulse(patternStartBeforeQ, cameraTrigger, (int)Parameters["Frame0TriggerDuration"], "cameraTrigger"); //camera trigger for first frame
    ...
    public override AnalogPatternBuilder GetAnalogPattern()
    {
        ...
        p.AddChannel("Rf3Amp");
        p.AddChannel("Rf4Amp");
        p.AddChannel("FeedforwardS");
        p.AddChannel("FeedforwardF");
        ...
        p.AddAnalogValue("Rf1Freq", motLoadEndTime, ((double)Parameters["SidebandFreq1Lambda"] - (double)Parameters["POS150OffsetFreq"]) / (double)Parameters["POS150Gradient"]);
        p.AddAnalogValue("Rf2Freq", motLoadEndTime, ((double)Parameters["SidebandFreq2Lambda"] - (double)Parameters["POS300OffsetFreq"]) / (double)Parameters["POS300Gradient"]);

        p.AddAnalogValue("Rf1Freq", cameraTrigger, ((double)Parameters["SidebandFreq1"] - (double)Parameters["POS150OffsetFreq"]) / (double)Parameters["POS150Gradient"]);
        p.AddAnalogValue("Rf2Freq", cameraTrigger, ((double)Parameters["SidebandFreq2"] - (double)Parameters["POS300OffsetFreq"]) / (double)Parameters["POS300Gradient"]);
        ...
        p.AddAnalogValue("Rf3Amp", motLoadEndTime, 0.0);
        p.AddAnalogValue("Rf4Amp", motLoadEndTime, 0.0);

        p.AddAnalogValue("Rf3Amp", cameraTrigger, (double)Parameters["SidebandAmp3"]);
        p.AddAnalogValue("Rf4Amp", cameraTrigger, (double)Parameters["SidebandAmp4"]);

        //Feedforward to cavities
        p.AddAnalogValue("FeedforwardS", 0, 0.0);
        
        p.AddAnalogValue("FeedforwardF", 0, 0.0);
