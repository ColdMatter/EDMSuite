...
public class Patterns : MOTMasterScript
{
    public Patterns()
    {
        ...
        */
        ...
        Parameters["FreqCVB1"] = 97.45; //+ F = 1Delta 97.45MHz 195.00MHz
        Parameters["FreqCVB3"] = 170.8; //- F = 2sig- 170.78MHz 341.56MHz
        Parameters["FreqCVB4"] = 172.3; //+ F = 2sig+ 170.48MHz 340.96MHz

        Parameters["FreqCVB4RAMPEND"] = 172.32; //+ F = 2sig+ 170.48MHz 340.96MHz

        Parameters["FreqCVB4RAMP_AMP"] = -0.00026;

        Parameters["BlueMOTField"] = 0.5;
        Parameters["BlueMOTRampDuration"] = 2000;
        Parameters["BlueMOTAMPRampDuration"] = 1000;
    ...
    public override Dictionary<string, List<List<double>>> GetDDSPattern()
    {
        ...
        int BlueMOTAMPRampEnd = BlueMOTRampEnd + (int)Parameters["BlueMOTAMPRampDuration"];
        int Recap = BlueMOTAMPRampEnd + (int)Parameters["BlueMOTHOLDDuration"];
        ...
        addDDSPattern(p, "FreqRampStart", BlueMOTRampEnd,
            (double)Parameters["FreqCVB1"], (double)Parameters["MOTFreqDDS2"], (double)Parameters["FreqCVB3"], (double)Parameters["FreqCVB4"],
            (double)Parameters["MOTAmpDDS1"], (double)Parameters["LightoffDDS2"], (double)Parameters["MOTAmpDDS3"], (double)Parameters["MOTAmpDDS4"],
            0.0, 0.0, 0.0, (double)Parameters["FreqCVB4RAMP_AMP"], 0.0, 0.0, 0.0, 0.0);
        
        addDDSPattern(p, "BlueMOTAMPRampEnd", BlueMOTAMPRampEnd,
            (double)Parameters["FreqCVB1"], (double)Parameters["MOTFreqDDS2"], (double)Parameters["FreqCVB3"], (double)Parameters["FreqCVB4RAMPEND"],
            (double)Parameters["MOTAmpDDS1"], (double)Parameters["LightoffDDS2"], (double)Parameters["MOTAmpDDS3"], (double)Parameters["MOTAmpDDS4"]);
    ...
    public override PatternBuilder32 GetDigitalPattern()
    {
        ...
        p.Pulse(0, Recap - (int)Parameters["Frame0TriggerDuration"], (int)Parameters["Frame0TriggerDuration"], "cameraTrigger");  //camera trigger imaging blue mot

        //p.Pulse(0, Recap, (int)Parameters["Frame0TriggerDuration"], "cameraTrigger"); //camera trigger for recap
    ...
    public override AnalogPatternBuilder GetAnalogPattern()
    {
        ...
        // B Field
        p.AddAnalogValue("MOTCoilsCurrent", 0, (double)Parameters["MOTCoilsCurrentValue"]); // on for mot

        p.AddLinearRamp("MOTCoilsCurrent", CompressRampDownStartTime, (int)Parameters["CompressRampDownDuration"], (double)Parameters["MOTCoilsCompressionValue"]); // ramp for compressed mot

        p.AddAnalogValue("MOTCoilsCurrent", lambdaCoolingStart, (double)Parameters["MOTCoilsOffValue"]); // switch off for molasses

        p.AddLinearRamp("MOTCoilsCurrent", BlueMOTRampStart, (int)Parameters["BlueMOTRampDuration"], (double)Parameters["BlueMOTField"]); // ramp for blue mot

        p.AddAnalogValue("MOTCoilsCurrent", Recap, (double)Parameters["MOTCoilsCurrentValue"]); // on for recap

        p.AddAnalogValue("MOTCoilsCurrent", (int)Parameters["MOTCoilsSwitchOff"], 0.0); // off at the end
