...
public class Patterns : MOTMasterScript
{
    public Patterns()
    {
        ...
        Parameters["v0IntensityImageValue"] = 8.75;
        ...
        Parameters["v0FrequencyImageValue"] = 6.0; //set this to MHz detuning desired if doing frequency jump (positive for blue detuning)
    ...
    public override AnalogPatternBuilder GetAnalogPattern()
    {
        ...
        p.AddAnalogValue("MOTCoilsCurrent", (int)Parameters["MOTSwitchOffTime"], 0.0);
        ...
        p.AddAnalogValue("v00Intensity", pumpTime, (double)Parameters["v0IntensityF0PumpValue"]);
        p.AddAnalogValue("v00Intensity", imageTime, (double)Parameters["v0IntensityImageValue"]);

        // v0 EOM
        p.AddAnalogValue("v00EOMAmp", 0, 7.17);
        p.AddAnalogValue("v00EOMAmp", pumpTime, 7.17);
        p.AddAnalogValue("v00EOMAmp", imageTime, 10.0);
        ...
        p.AddAnalogValue(
            "v00Frequency",
            imageTime,
            ((double)Parameters["lockAomFrequency"] - (double)Parameters["v0FrequencyImageValue"] / 2 - (double)Parameters["calibOffset"]) / (double)Parameters["calibGradient"]
        );
