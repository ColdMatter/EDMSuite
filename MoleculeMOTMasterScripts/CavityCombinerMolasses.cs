...
public class Patterns : MOTMasterScript
{
    public Patterns()
    {
        ...
        Parameters["SidebandFreqJump2"] = 20.0 / 2.0; //- F = 2
        Parameters["SidebandFreqJump3"] = 10.0 / 2.0; //+ F = 0
        Parameters["SidebandFreqJump4"] = 10.0 / 2.0; //+ F = 1+

        //Timing
        Parameters["MotLoadTime"] = 4000;
        Parameters["MolassesRampTime"] = 100;
    ...
    public override PatternBuilder32 GetDigitalPattern()
    {
        ...
        int motEndTime = patternStartBeforeQ + (int)Parameters["MotLoadTime"];
        int molassesStartTime = motEndTime + (int)Parameters["MolassesRampTime"];
        ...
        p.Pulse(patternStartBeforeQ, imageTime, (int)Parameters["Frame0TriggerDuration"], "cameraTrigger"); //camera trigger for first frame
    ...
    public override AnalogPatternBuilder GetAnalogPattern()
    {
        ...
        //p.AddAnalogValue("Rf1Freq", 0, ((double)Parameters["SidebandFreqMolasses1"] - (double)Parameters["SidebandFreqJump1"] - (double)Parameters["POS150OffsetFreq"]) / (double)Parameters["POS150Gradient"]);
        //p.AddAnalogValue("Rf2Freq", 0, ((double)Parameters["SidebandFreqMolasses2"] - (double)Parameters["SidebandFreqJump2"] - (double)Parameters["POS300OffsetFreq"]) / (double)Parameters["POS300Gradient"]);
        //p.AddAnalogValue("Rf3Freq", 20000, ((double)Parameters["SidebandFreq3"] - (double)Parameters["SidebandFreqJump3"] - (double)Parameters["POS300OffsetFreq"]) / (double)Parameters["POS300Gradient"]);
        p.AddAnalogValue("Rf4Freq", 20000, ((double)Parameters["SidebandFreq4"] - (double)Parameters["SidebandFreqJump4"] - (double)Parameters["POS300OffsetFreq"]) / (double)Parameters["POS300Gradient"]);
        ...
        p.AddAnalogValue("FeedforwardS", 20000, 0.0 + (double)Parameters["FeedforwardFactorS"] * (double)Parameters["SidebandFreqJump4"]);

        p.AddAnalogValue("FeedforwardF", 0, 0.0);
        p.AddAnalogValue("FeedforwardF", 20000, 0.0 + (double)Parameters["FeedforwardFactorF"] * (double)Parameters["SidebandFreqJump3"]);
