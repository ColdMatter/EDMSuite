...
namespace NavigatorHardwareControl
{
    public partial class voltageLogger : Form
    {
        ...
        private Task voltageInputTask;
        private Task continuousTask;
        private Task runningTask;
        private AnalogMultiChannelReader VIReader;
        ...
        public double[,] acquireAnalogInputData(string physicalChan,string taskName,double clockRate)
        {
            ...
            voltageInputTask.Control(TaskAction.Verify);
        ...
        {
            voltageInputTask = new Task();

            voltageInputTask.AIChannels.CreateVoltageChannel(physicalChan, taskName,AITerminalConfiguration.Differential,
            (double)0.0, (double)10.0, AIVoltageUnits.Volts);

            voltageInputTask.Timing.ConfigureSampleClock("",clockRate,SampleClockActiveEdge.Rising,SampleQuantityMode.FiniteSamples);

            VIReader = new AnalogMultiChannelReader(voltageInputTask.Stream);
        }
        ...
        private void logBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    VIReader = new AnalogMultiChannelReader(continuousTask.Stream);
