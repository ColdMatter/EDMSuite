Mode DDS50 (50-tone DDS)
This card mode requires the SPCM_FEAT_EXTFW_DDS50 feature to be installed and a DDS50 capable firmware
image to be active. For details see “Feature and firmware matrix of AWG and DDS products” paragraph in
...
Table 174: Spectrum API: card mode and read out of available card mode software registers
...
(c) Spectrum Instrumentation GmbH 169

Mode DDS50 (50-tone DDS) DDS Command buffer fill size, overrun and underrun
Command FIFO
...
Image 85: command queue, trigger and timer interaction
...
the returned value of SPC_DDS_QUEUE_CMD_COUNT Image 86: The different FIFO buffers in DDS mode
...
(c) Spectrum Instrumentation GmbH 170
...
Table 175: Spectrum API: DDS information on the command queue
Register Value Direction Description
...
mand.
DDS Command
Table 176: Spectrum API: DDS command register
Register Value Direction Description
...
Table 177: Spectrum API: DDS trigger sources
Register Value Direction Description
...
Table 179: Spectrum API: DDS information registers
Register Value Direction Description
...
Table 180: Spectrum API: DDS trigger timer
Register Value Direction Description
...
(c) Spectrum Instrumentation GmbH 172

Mode DDS50 (50-tone DDS) DDS Command buffer fill size, overrun and underrun
DDS50 core connections
The DDS_CORE_ON_CHx registers defines the
setup of the different multiplexers that form the
connections of the different DDS cores on hard-
ware channels. If a hardware channel is not pres-
ent on your specific hardware, programming this
connection does not have any function. In that
case, dedicated DDS cores that are routed to a
fixed hardware channel (like Core 49 being sole-
ly connected to hardware channel 3) cannot be
used.
Each block represents a single DDS core or a
group of DDS cores where the following param-
eters can be individually programmed:
• Amplitude
• Frequency
• Phase
• Amplitude Slope
• Frequency Slope
Image 87: block diagram of the DDS50 option showing the DDS cores and connections options.
The cores on channel registers combine the setup of all shown multiplexers. Each core is represented by a single bit. All cores that should be
routed to the dedicated channel needs to be combined by a logical OR. Only settings that match the shown possible connections in the above
drawing can be selected:
Table 181: Spectrum API: DDS50 connection register
| Register             | Value Direction    | Description                                |
| -------------------- | ------------------ | ------------------------------------------ |
...
| ...                  | ...                | ...                                        |
| SPCM_DDS_CORE45      | 200000000000h      | core 45 connected to channel.              |
| SPCM_DDS_CORE46      | 400000000000h      | core 46 connected to channel.              |
| SPCM_DDS_CORE47      | 800000000000h      | core 47 connected to channel.              |
| SPCM_DDS_CORE48      | 1000000000000h     | core 48 connected to channel.              |
| SPCM_DDS_CORE49      | 2000000000000h     | core 49 connected to channel.              |
...
// connect the maximum number of 47 cores to channel 0 and the minimum to channel 1, 2 and 3
spcm_dwSetParam_i64 (hCard, SPC_DDS_CORES_ON_CH0, 0x7FFFFFFFFFFF);
spcm_dwSetParam_i64 (hCard, SPC_DDS_CORES_ON_CH1, SPCM_DDS_CORE47);
spcm_dwSetParam_i64 (hCard, SPC_DDS_CORES_ON_CH2, SPCM_DDS_CORE48);
spcm_dwSetParam_i64 (hCard, SPC_DDS_CORES_ON_CH3, SPCM_DDS_CORE49);

(c) Spectrum Instrumentation GmbH 173

Mode DDS50 (50-tone DDS) Programming the DDS cores
...
Image 89: Details of the DDS core
...
Table 182: Spectrum API: DDS core frequency settings
...
Table 183: Spectrum API: DDS programmable frequency range
Register Value Direction Type Description
...
Table 184: Spectrum API: DDS core amplitude settings
Register Value Direction Type Description
...
(c) Spectrum Instrumentation GmbH 174
...
Table 185: Spectrum API: DDS programmable amplitude range
Register Value Direction Type Description
...
Table 186: Spectrum API: DDS core phase settings
Register Value Direction Type Description
...
Table 187: Spectrum API: DDS programmable phase range
Register Value Direction Type Description
...
(c) Spectrum Instrumentation GmbH 175
...
Table 188: Spectrum API: DDS core frequency slope settings
Register Value Direction Type Description
...
Table 189: Spectrum API: DDS programmable frequency slope range
Register Value Direction Type Description
...
Table 190: Spectrum API: DDS core amplitude slope settings
Register Value Direction Type Description
...
Table 192: Spectrum API: DDS core slope step size settings
Register Value Direction Type Description
...
(c) Spectrum Instrumentation GmbH 177

Mode DDS50 (50-tone DDS) Defining the Phase Behaviour
...
Table 193: Spectrum API: DDS Phase Behaviour
Register Value Direction Description
...
SPC_DDS_CMD: SPCM_DDS_CMD_EXEC_AT_TRG
Image 90: Phase Jumps to 90° instantly when the 2nd trigger arrives, regardless of prior state.
Mode Phase Shift
...
SPC_DDS_CMD: SPCM_DDS_CMD_EXEC_AT_TRG
// 2. Sequence
Image 91: Phase shifts forward by 90° relative to current state at 2nd Trigger
SPC_DDS_CORE0_PHASE: 270°
SPC_DDS_CMD: SPCM_DDS_CMD_EXEC_AT_TRG
(c) Spectrum Instrumentation GmbH 178

Mode DDS50 (50-tone DDS) Additional functions in DDS core
...
Table 194: Spectrum API: multi-purpose I/O lines registers and available register settings
Register Value Direction Description
...
Table 195: Spectrum API: DDS multi-purpose I/O additional registers
Register Value Direction Description
...
Table 196: Spectrum API: DDS multi-purpose I/O manual output register
Register Value Direction Description
...
spcm_dwSetParam_i32 (hCard, SPC_DDS_CMD, SPCM_DDS_CMD_EXEC_AT_TRG);
spcm_dwSetParam_i32 (hCard, SPC_DDS_CMD, SPCM_DDS_CMD_WRITE_TO_CARD);
(c) Spectrum Instrumentation GmbH 179

Mode DDS50 (50-tone DDS) Improving DDS throughput by writing commands as parameter lists
...
Table 197: Spectrum API: register lists register
Register Value Direction Type Description
...
(c) Spectrum Instrumentation GmbH 180

Mode DDS50 (50-tone DDS) Defining the data transfer mode
Defining the data transfer mode
...
Table 198: Spectrum API: data transfer mode definition
Register Value Direction Description
...
er will automatically insert NOP (no operation = dummy) Image 92: The different FIFO buffers in DDS mode
...
Table 199: Comparison of the different DDS FIFOs and their fillsize registers
...
(c) Spectrum Instrumentation GmbH 181

Mode DDS50 (50-tone DDS) Execute Now, Timer and Trigger timing behaviour
Below is a comparison of the two different modes in terms of advantages and disadvantages:
Table 200: Comparison of single and DMA transfer mode
DTM_SINGLE DTM_DMA
...
(c) Spectrum Instrumentation GmbH 182

Pulse Generator (Firmware Option) General Information
