 ...
 * */
{
    public Patterns()
    {
        ...
        Parameters["yagONorOFF"] = 10.0; // this pattern has an additional yag shot at the reset time
        ...
        Parameters["V00ShutterONorOFF"] = 10.0;
        ...
        Parameters["MOTRecaptureHoldDuration"] = 1000;
    ...
    public override PatternBuilder32 GetDigitalPattern()
    {
        ...
        int motRecaptureStart = odtStop + (int)Parameters["MOTRecaptureStartDelay"];
        int motRecaptureStop = motRecaptureStart + (int)Parameters["MOTRecaptureHoldDuration"];
        int imagingLightStart = motRecaptureStop + (int)Parameters["CameraTriggerStartDelay"];
        ...
        if ((double)Parameters["yagONorOFF"] > 5.0)
        {
            ...
            p.Pulse(
                patternStartBeforeQ,
                (int)Parameters["ResetTime"] - (int)Parameters["FlashToQ"],
                (int)Parameters["QSwitchPulseDuration"],
                "flash"
            );

            p.Pulse(patternStartBeforeQ, (int)Parameters["ResetTime"], (int)Parameters["QSwitchPulseDuration"], "q");
        ...
        // V00 shutters
        if ((double)Parameters["V00ShutterONorOFF"] > 5.0)
        {
            p.Pulse(
                patternStartBeforeQ,
                blueMOTStop - 10,
                odtStop - blueMOTStop + 10 - 1500,
                "RbPushBeamAOM"
            );
        }
