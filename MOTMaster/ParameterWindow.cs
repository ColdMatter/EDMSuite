...
namespace MOTMaster
{
    public partial class ParameterWindow : Form
    {
        ...
        // Groups currently collapsed by the user (see COLLAPSIBLE GROUP HEADERS below).
        // Keyed by ListViewGroup instance, so this resets naturally whenever
        // RebuildListView recreates the groups (e.g. on file reload).
        private readonly HashSet<ListViewGroup> _collapsedGroups = new HashSet<ListViewGroup>();
        private GroupHeaderClickWindow _groupCollapseClickWindow;
        ...
        public ParameterWindow()
        {
            ...
            lvParameters.HandleCreated += (s, e) => AttachGroupCollapseClickWindow();
            lvParameters.HandleDestroyed += (s, e) => DetachGroupCollapseClickWindow();
            if (lvParameters.IsHandleCreated) AttachGroupCollapseClickWindow();
        }
        ...
        // chevron - but comctl32 does NOT toggle the collapsed state by itself
        // on click. A real Win32 app (or WinForms .NET 5+, which added exactly
        // this in dotnet/winforms#3155) has to hit-test the click with
        // LVM_HITTEST and re-issue LVM_SETGROUPINFO with LVGS_COLLAPSED flipped.
        // .NET Framework's ListView.WndProc never does that, so without the
        // GroupHeaderClickWindow subclass below the chevron renders but does
        // nothing when clicked. Once the state bit does get toggled, comctl32
        // itself handles hiding/showing the group's items - no manual item
        // visibility bookkeeping needed here.
        //
        ...
        private const uint LVHT_EX_GROUP_COLLAPSE = 0x40000000;
        ...
        private void MakeGroupCollapsible(ListViewGroup group) => SetGroupCollapsedState(group, false);
        ...
        private void SetGroupCollapsedState(ListViewGroup group, bool collapsed)
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.WriteLine($"ParameterWindow: could not set collapsed state for group \"{group.Header}\": {ex}");
            }
        }
        ...
        // Detects clicks on a group's chevron (single click) or header (double
        // click) and flips its collapsed state - see the class-level remarks
        // above for why this hook is necessary at all under .NET Framework.
        private void AttachGroupCollapseClickWindow()
        {
            DetachGroupCollapseClickWindow();
            _groupCollapseClickWindow = new GroupHeaderClickWindow(lvParameters, OnGroupHeaderClicked);
        }

        private void DetachGroupCollapseClickWindow()
        {
            _groupCollapseClickWindow?.ReleaseHandle();
            _groupCollapseClickWindow = null;
        }

        private void OnGroupHeaderClicked(Point clientPoint, bool isDoubleClick)
        {
            if (!lvParameters.IsHandleCreated) return;

            var hitInfo = new LVHITTESTINFO { pt = clientPoint };
            IntPtr result = SendMessage(lvParameters.Handle, LVM_HITTEST, (IntPtr)(-1), ref hitInfo);
            int groupId = unchecked((int)result.ToInt64());
            if (groupId == -1) return;

            bool chevronClicked = !isDoubleClick && (hitInfo.flags & LVHT_EX_GROUP_COLLAPSE) != 0;
            bool headerDoubleClicked = isDoubleClick && (hitInfo.flags & LVHT_EX_GROUP_HEADER) != 0;
            if (!chevronClicked && !headerDoubleClicked) return;

            foreach (ListViewGroup group in lvParameters.Groups)
            {
                if (GetNativeGroupId(group) != groupId) continue;

                bool collapsed = !_collapsedGroups.Contains(group);
                if (collapsed)
                    _collapsedGroups.Add(group);
                else
                    _collapsedGroups.Remove(group);

                SetGroupCollapsedState(group, collapsed);
                break;
            }
        }

        // Stock WinForms ListView.WndProc never looks for group-header clicks
        // (that's the whole bug - see remarks above), so this subclasses the
        // control's own window to observe WM_LBUTTONUP/WM_LBUTTONDBLCLK without
        // disturbing any of the control's normal message handling: WndProc's
        // default base implementation forwards everything it doesn't touch to
        // the original window procedure via DefWndProc.
        private sealed class GroupHeaderClickWindow : NativeWindow
        {
            private const int WM_LBUTTONUP = 0x0202;
            private const int WM_LBUTTONDBLCLK = 0x0203;
            private const int WM_NCDESTROY = 0x0082;

            private readonly Action<Point, bool> _onClick;

            public GroupHeaderClickWindow(Control control, Action<Point, bool> onClick)
            {
                _onClick = onClick;
            ...
            protected override void WndProc(ref Message m)
            {
                ...
                switch (m.Msg)
                {
                    ...
                    case WM_LBUTTONDBLCLK:
                        ...
                        _onClick(point, m.Msg == WM_LBUTTONDBLCLK);
                        break;
