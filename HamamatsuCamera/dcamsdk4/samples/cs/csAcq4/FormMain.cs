...
namespace csAcq4
{
    public partial class FormMain : Form
    {
        ...
        private void auto_lut()
        {
            ...
            //    if (m_lut.inmax == m_lut.inmin)
            //    {
            //        Console.WriteLine("Auto LUT failed: All pixels have the same value.");
            //        // Fallback: Apply forced adjustment
            //        m_lut.inmax += 50;
            //        m_lut.inmin -= 50;

            //        if (m_lut.inmin < 0) m_lut.inmin = 0;
            //        if (m_lut.inmax > m_lut.cameramax) m_lut.inmax = m_lut.cameramax;

            //        Console.WriteLine($"Forcing LUT adjustment: min={m_lut.inmin}, max={m_lut.inmax}");
            //        bUpdatePicture = true;
            //    }
        ...
        //    if (oldImg != null)
        //        oldImg.Dispose();
        //    Console.WriteLine("PictureBox updated.");
        //}
        ...
        private bool ConfigureCamera()
        {
            try
            {
                ...
                // Set exposure time to 4.32ms
                MyDcamProp exposuretimeprop = new MyDcamProp(mydcam, DCAMIDPROP.EXPOSURETIME);
                if (!exposuretimeprop.setvalue(0.00432)) // Adjust exposure time in seconds
                {
                ...
                // Set trigger source to internal (no molecules, no external trigger)
                ...
                {
                    Console.WriteLine("Trigger source set to internal.");
                }
                else
                {
                    MyShowStatusNG("Failed to set trigger source to INTERNAL", triggerSourceProp.m_lasterr);
                    return false;  // Fail: setting trigger source to internal
                }

                // Switch to EXTERNAL mode when there are molecules running
                bool useExternalTrigger = false;  // Set this variable as needed

                if (useExternalTrigger)
                {
                    if (triggerSourceProp.setvalue(DCAMPROP.TRIGGERSOURCE.EXTERNAL))
                    {
                        Console.WriteLine("Trigger source set to external.");
                    }
                    else
                    {
                        MyShowStatusNG("Failed to set trigger source to EXTERNAL", triggerSourceProp.m_lasterr);
                        return false;  // Fail: setting trigger source to external
                    }
                }


                // Set trigger mode to start
        ...
        private void QuerySensorTemperatureButton_Click(object sender, EventArgs e)
        {
            ...
            if (sensortemperatureprop.getvalue(ref currentTemperature))
            {
                Console.WriteLine($"Current Sensor Temperature: {currentTemperature} degrees");
                SensorTemperatureLabel.Text = $"Current Sensor Temperature: {currentTemperature} degrees";
            }
            else
            {
        ...
        private void QuerySensitivityGainButton_Click(object sender, EventArgs e)
        {
            ...
            if (sensitivitygainprop.getvalue(ref currentSensitivity))
            {
                ...
                SensitivityGainLabel.Text = $"Current Sensitivity Gain: {currentSensitivity}";
            }
            else
            {
        ...
        {
            if (double.TryParse(SensitivityGainTextBox.Text, out double newGain))
            {
                MyDcamProp sensitivitygainprop = new MyDcamProp(mydcam, DCAMIDPROP.SENSITIVITY);
                ...
                if (sensitivitygainprop.setvalue(newGain))
                {
                    ...
                    SensitivityGainLabel.Text = $"Updated Sensitivity Gain: {newGain}";
                }
                else
                {
        ...
        private void QueryExposureTimeButton_Click(object sender, EventArgs e)
        {
            ...
            {
                Console.WriteLine($"Current Exposure Time: {currentExposureTime}");
                ExposureTimeLabel.Text = $"Current Exposure Time: {currentExposureTime} seconds";
            }
            else
            {
        ...
        {
            if (double.TryParse(ExposureTimeTextBox.Text, out double newExposureTime))
            {
                MyDcamProp exposuretimeprop = new MyDcamProp(mydcam, DCAMIDPROP.EXPOSURETIME);
                ...
                {
                    Console.WriteLine($"Updated Exposure Time to {newExposureTime}");
                    ExposureTimeLabel.Text = $"Updated Exposure Time: {newExposureTime} seconds";
                }
                else
                {
        ...
        private void PushSnap_Click(object sender, EventArgs e)
        {
            ...
            if (mydcam.cap_stop()) 
            {
                MyShowStatusOK("Snap operation completed");
            ...
            {
                MyShowStatus("Image failed to load in PictureBox.");
            }
            else
            {
                MyShowStatus("Image successfully updated in PictureBox.");
            }
        ...
        private void PushLive_Click(object sender, EventArgs e)
        {
            ...
            if (IsMyFormStatus_Initialized())
            {
                ...
                update_lut(false);  // Success: update LUT
            }
        ...
        // Idle button stops image acquisition and return the application to a state where the camera is not actively capturing or processing frames.
