...
namespace RbCaFHardwareControl
{
    ...
    public class Controller : MarshalByRefObject, CameraControllable, ExperimentReportable
    {
        ...
        private static string cameraAttributesPath = (string)Environs.FileSystem.Paths["HardwareControllerCameraAttributesPath"];
        private static string profilesPath = (string)Environs.FileSystem.Paths["settingsPath"]
            + "RbCaFHardwareController\\";
  ...
  //      voltageLogger voltageLoggerWindow;
        ...
        internal void ControllerLoaded()
        {
            ...
            {
                MessageBox.Show("Could not find StoppedParameters.bin in path " + profilesPath + ", channels reverting to default values.");
                loadedState = generateFreshHardwareState();
            }
        ...
        private hardwareState generateFreshHardwareState()
        {
            hardwareState newHardwareState = new hardwareState();
            newHardwareState.analogs = new Dictionary<string, double>();
            newHardwareState.digitals = new Dictionary<string,bool>();
            foreach (string analogChan in analogTasks.Keys)
            {
                newHardwareState.analogs[analogChan] = (double)0.0;
            }
            foreach (string digitalChan in digitalTasks.Keys)
            {
                newHardwareState.digitals[digitalChan] = false;
            }
        ...
        #region TCPServer for frequency monitor
        //The idea is to have a tcp server to allow other computers to receive information from the frequency counter, if desired.
        //There should be a tcp listener which will accept connections from certain permitted hostnames.
        //The hardware controller can periodically write data to the tcp socket to be parsed by the client
        private bool listenForTCPClients = false;
        private void startTcpListener()
        {
            //I'm not sure where the correct place to put the client is....
            TcpClient client = new TcpClient();
            TcpListener listener = new TcpListener(IPAddress.Parse("155.198.206.215"),13000);
            listener.Start();
            listenForTCPClients = true;
            while (listenForTCPClients)
            {
                client = listener.AcceptTcpClient();
                NetworkStream tcpStream = client.GetStream();
                byte[] msgToSend = System.Text.Encoding.ASCII.GetBytes("TCPServer is sending data..");
                tcpStream.Write(msgToSend, 0, msgToSend.Length);
            }
        }
