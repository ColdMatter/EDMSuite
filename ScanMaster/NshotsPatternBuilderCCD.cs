...
namespace ScanMaster.Acquire.Patterns
{
    ...
    /// </summary>
    public class NshotsPatternBuilderCCD : DAQ.Pattern.PatternBuilder32
    {
        public NshotsPatternBuilderCCD()
        {
        }
        ...
        public int ShotSequence(int startTime, int NumberOfShots, int flashlampPulseInterval, int flashToQ, int flashlampPulseLength,
            ...
            int delayToDetectorTrigger, int delayToDetectorTriggerCCD, int ttlSwitchPort, int ttlSwitchLine, int switchLineDuration, int switchLineDelay, bool modulation)
        {
            ...
            for (int ShotIndex = 0; ShotIndex < NumberOfShots; ShotIndex++)
            {
                ...
                if (ShotIndex % 2 == 1)
                {
                    ...
                    if (modulation)
                    {
                        Shot(time, flashToQ, flashlampPulseLength, delayToDetectorTrigger, delayToDetectorTriggerCCD, "detectorprime", "ccdtrigger");
                    }
                    else
                    {
                        Pulse(time, switchLineDelay, switchLineDuration, switchChannel);
            ...
            public int TTLpulses(int time, int[] ttlStartTimes, int[] ttlDurations, int[] ttlRepetitions, int numberOfShots, int ShotIndex, int flashlampPulseInterval, int channel)
        {
            ...
            public int Shot(int startTime, int flashToQ, int flashlampPulseLength, int delayToDetectorTrigger, int delayToDetectorTriggerCCD, string detectorTriggerSource, string CCDTriggerSource)
        {
            int time = 0;
            int tempTime = 0;
            // Flash pulse
            ...
            if (tempTime > time) time = tempTime;
            // CCD enable trigger
            tempTime = Pulse(startTime, delayToDetectorTriggerCCD + flashToQ, DETECTOR_TRIGGER_LENGTH,
                ((DigitalOutputChannel)Environs.Hardware.DigitalOutputChannels[CCDTriggerSource]).BitNumber);
            if (tempTime > time) time = tempTime;
