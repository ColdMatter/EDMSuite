...
namespace ScanMaster.Acquire.Patterns
{
    ...
    public class ZeemanSisyphusPatternBuilder : DAQ.Pattern.PatternBuilder32
    {
        ...
        public int ShotSequence(int startTime, int numberOfOnOffShots, int padShots, int flashlampPulseInterval,
            int valvePulseLength, int valveToQ, int flashToQ, int flashlampPulseLength,  int pmtTrigger, bool modulation)
        {
            int time = startTime;
            ...
            for (int i = 0; i < numberOfOnOffShots; i++)
            {
                ...
                Shot(time, valvePulseLength, valveToQ, flashToQ, flashlampPulseLength, pmtTrigger, true);
                ...
                if (modulation)
                {
                    Shot(time, valvePulseLength, valveToQ, flashToQ, flashlampPulseLength, pmtTrigger, false);
                    time += flashlampPulseInterval;
                    for (int p = 0; p < padShots; p++)
                    {
                        FlashlampPulse(time, valveToQ, flashToQ, flashlampPulseLength);
        ...
        public int Shot(int startTime, int valvePulseLength, int valveToQ, int flashToQ, int flashlampPulseLength, int pmtTrigger, bool qTrig)
        {
            int time = 0;
            int tempTime = 0;
            ...
            tempTime = Pulse(startTime, valveToQ + pmtTrigger, CAMERA_TRIGGER_LENGTH,
                ((DigitalOutputChannel)Environs.Hardware.DigitalOutputChannels["pmtTrigger"]).BitNumber);
            if (tempTime > time) time = tempTime;
