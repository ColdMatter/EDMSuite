...
namespace ScanMaster.Acquire.Patterns
{
    ...
    /// </summary>
    public class STIRAPpatternBuilder : DAQ.Pattern.PatternBuilder32
    {
        public STIRAPpatternBuilder()
        {
        }
        ...
        //Note: number of shots paramter is the "sequence length" in scan master
            ...
            int delayToDetectorTrigger, int ttlSwitchPort, int ttlSwitchLine, int switchLineDuration, int switchLineDelay, bool modulation)
        {
            ...
            {
                if (modulation & (ShotIndex % 2 != 0))//e.g 1,3,5 etc
                {
                    ...
                    //}
                    Pulse(time, switchLineDelay, switchLineDuration, ((DigitalOutputChannel)Environs.Hardware.DigitalOutputChannels["ttl2"]).BitNumber);
