    ...
    //	private const int FLASH_PULSE_LENGTH = 100;
			...
			int ttlSwitchPort, int ttlSwitchLine, int switchLineDuration, int switchLineDelay, int chirpStart, int chirpDuration, bool modulation) 
		{
			...
			for (int i = 0 ; i < numberOfOnOffShots ; i++ ) 
			{
                ...
                Shot(time, valvePulseLength, valveToQ, flashToQ, flashlampPulseLength, aomStart1, aomDuration1, aomStart2, aomDuration2, delayToDetectorTrigger, chirpStart, chirpDuration, "detector");
                ...
                if (modulation)
                {
                    Shot(time, valvePulseLength, valveToQ, flashToQ, flashlampPulseLength, aomStart1, aomDuration1, aomStart2, aomDuration2, delayToDetectorTrigger, chirpStart, chirpDuration, "detectorprime");
                    time += flashlampPulseInterval;
                    for (int p = 0; p < padShots; p++)
                    {
                        FlashlampPulse(time, valveToQ, flashToQ, flashlampPulseLength);
		...
		public int Shot( int startTime, int valvePulseLength, int valveToQ, int flashToQ, int flashlampPulseLength, int aomStart1, int aomDuration1,
            int aomStart2, int aomDuration2, int delayToDetectorTrigger, int chirpStart, int chirpDuration, string detectorTriggerSource)  
		{
			int time = 0;
			int tempTime = 0;
