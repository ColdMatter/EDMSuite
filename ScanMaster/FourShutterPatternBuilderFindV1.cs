...
namespace ScanMaster.Acquire.Patterns
{
	...
	/// </summary>
	public class FourShutterPatternBuilderFindV1 : DAQ.Pattern.PatternBuilder32
	{
		public FourShutterPatternBuilderFindV1()
		{
		}
			...
			int shutterV1delay, int shutterV2delay, int V2OnTime, int DurationV1, bool modulation)
		{
			...
			for (int i = 0; i < shots; i++)
			{
				...
				if (modulation & (i % 2 != 0))//e.g 1,3,5 etc
				{
					...
					Pulse(time, shutterslowdelay - padStart, DurationV0, ((DigitalOutputChannel)Environs.Hardware.DigitalOutputChannels["shutter2on"]).BitNumber);
					...
					//Pulse(time, shutterV1delay-padStart, V1OnTime, ((DigitalOutputChannel)Environs.Hardware.DigitalOutputChannels["shutterv1"]).BitNumber);
				...
				else //else do a normal shot with the switch line high. e.f 0,2,4,6 it hits this one first
				{
					...
					Pulse(time, shutterslowdelay-padStart, DurationV0, ((DigitalOutputChannel)Environs.Hardware.DigitalOutputChannels["shutter2on"]).BitNumber);//think there something up with this one added time BQ - want shutter to open before the shot fires 9 what about valve to q
