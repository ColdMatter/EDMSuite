...
BeginPackage["SEDM3`Polarisation`"];
...
SEDM3`Polarisation`\[Alpha][\[Lambda]_]=-D[groundState[y],y]/.y->\[Lambda];
SetDirectory[$TopDirectory<>"\\AddOns\\Applications\\"];
DumpSave["polSEDM3.mx",SEDM3`Polarisation`\[Alpha]];
...
Off[Get::"noopen"];
If[Get["polSEDM3.mx"]==$Failed,Print["Calculating rotor Stark shift. First time only."];calculateAndCachePolarisation[];Get["polSEDM2.mx"];Print["Cached for next time"]];
On[Get::"noopen"];


polarisationFactor[EField_]:=SEDM3`Polarisation`\[Alpha][(3.91*3.335 * 10^-30 * EField*10^5)/(7.24*10^9*6.626*10^-34)]
