...
plotClusterDiagnostics[clusterName_,detectorAndChannelList_]:=Module[{bl,analysedBlocks,channelPlots,noisePlot,errorPlot,gridWidth,sourceNoisePlot,sourceNoisePlotRunning,b\[Alpha]ePlots},
bl=getDBlock/@uidsForAnalysisTag["fwhm",uidsForCluster[clusterName]];
...
(*plotShots[block_,gates_]:=Module[{dataList,plotTOF,pointMax,detectorMax,integralArrays},dataList[point_,detector_]:=({#1[Times],#1[Data]}&)[block[Points[point][Shot[TOFs[detector]]]]];plotTOF[i_]:=Module[{},ListPlot[block[GetAverageTOF[i][Data]],Joined->True,DisplayFunction->Identity]];pointMax=block[Points[Count]]-1;detectorMax=block[Points[0][Shot[TOFs[Count]]]]-1;integralArrays=(block[GetTOFIntegralArray[#1,gates[[#1+1]][[1]],gates[[#1+1]][[2]]]]&)/@Range[0,detectorMax];Show[GraphicsRow[{(ListPlot[#1,DisplayFunction->Identity]&)/@integralArrays,plotTOF/@Range[0,detectorMax]}],DisplayFunction->Identity]];*)


(*plotPulsedRFGates[block_,rf1KeepLength_,rf2KeepLength_,offset_]:=Module[{dataList,config,gates,pls,gatpllow,gatplhi},dataList[point_,detector_]:=({#1[Times],#1[Data]}&)[block[Points[point][Shot[TOFs[detector]]]]];config=extractConfig[block];gates=generatePulsedRFGates["rf1CentreTime"/.config,"rf2CentreTime"/.config,rf1KeepLength,rf2KeepLength,offset];pls=(ListPlot[Transpose[dataList[#1,0]],Joined->True,PlotStyle->GrayLevel[#1/25],DisplayFunction->Identity]&)/@Range[0,25];gatpllow=ParametricPlot[{gates[[1]],t},{t,0,5},PlotStyle->{Red,Thickness[0.003`],Dashing[{0.02`}]},DisplayFunction->Identity];gatplhi=ParametricPlot[{gates[[2]],t},{t,0,5},PlotStyle->{Blue,Thickness[0.003`],Dashing[{0.02`}]},DisplayFunction->Identity];Show[pls,gatpllow,gatplhi,Background->GrayLevel[0.6`],AspectRatio->1/3,DisplayFunction->Identity]];*)
...
PlotRange->{{0,1},{0,yMax}},PlotLabel->"Source noise spectrum (ppt)"(*,AxesLabel->{"Fraction of rep. rate",""}*)
]]
