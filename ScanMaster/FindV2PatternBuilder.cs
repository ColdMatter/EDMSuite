...
namespace ScanMaster.Acquire.Patterns
{
	...
	/// </summary>
	public class FindV2PatternBuilder : DAQ.Pattern.PatternBuilder32
	{
		public FindV2PatternBuilder()
		{
		}
			...
			int shutterV1delay, int shutterV2delay, int DurationV2, int DurationV1, bool modulation,int switchLineDelay,int shutter1offdelay,int v3delaytime, int DurationIR)
		{
			...
			for (int i = 0; i < shots; i++)
			{
				...
				if (modulation & (i % 2 != 0))//e.g 1,3,5 etc
				{
					...
					Pulse(time, DurationIR, shutterPulseLength, ((DigitalOutputChannel)Environs.Hardware.DigitalOutputChannels["ttl4"]).BitNumber); // this is the IR shutter


					Pulse(time, shutterV1delay - padStart, DurationV1, ((DigitalOutputChannel)Environs.Hardware.DigitalOutputChannels["ttl2"]).BitNumber);
					...
					Pulse(time, shutterV2delay, DurationV2, ((DigitalOutputChannel)Environs.Hardware.DigitalOutputChannels["ttl3"]).BitNumber);
				...
				else //else do a normal shot with the switch line high. e.f 0,2,4,6 it hits this one first
				{
					...
					Pulse(time, DurationIR, shutterPulseLength, ((DigitalOutputChannel)Environs.Hardware.DigitalOutputChannels["ttl4"]).BitNumber);
					//IR shutter
					//Pulse(time, valveToQ - switchLineDelay, shutterPulseLength, switchChannel); // This is just a digital output ttl. This is used for the opening pulse of the IR shutter. The newport ones need a pulse to turn on and one to turn off
