...
namespace ScanMaster.Acquire.Patterns
{
	...
	public class TwoShutterPatternBuilder : DAQ.Pattern.PatternBuilder32
	{
		...
		public int ShotSequence(int startTime, int shots, int padShots, int padStart, int flashlampPulseInterval,
			...
			int ttlSwitchPort, int ttlSwitchLine, int switchLineDuration, int switchLineDelay, int shutteroffdelay, int shutter1offdelay, bool modulation)
		{
			int time;
			if (padStart == 0)
			{
			...
			for (int i = 0; i < shots; i++)
			{
				...
				{
					Pulse(startTime, 0, ShutterPulseLength, ((DigitalOutputChannel)Environs.Hardware.DigitalOutputChannels["shutter2on"]).BitNumber);//think there something up with this one added time BQ - want shutter to open before the shot fires 9 what about valve to q
					Shot(time, valvePulseLength, valveToQ, flashToQ, flashlampPulseLength, delayToDetectorTrigger, "detectorprime");
					Pulse(time, shutteroffdelay, ShutterPulseLength, ((DigitalOutputChannel)Environs.Hardware.DigitalOutputChannels["shutter2off"]).BitNumber);//seems to turn off at the next pattern needs to be negative not sure why
