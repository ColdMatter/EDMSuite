...
def Acquire():
	...
	# start looping
	sm.AdjustProfileParameter("out", "channel", "VCO155Freq", False)
	hc.UpdateTopProbeMicrowaveMixerV(0)
	for j in range(10):
		for i in range (3):
			hc.UpdateBottomProbeMicrowaveMixerV(0.7)
			#hc.SwitchMwAndWait(True)
			hc.SwitchMwAndWait(False)#if molecules are pumped into f=1
			bh.StartPattern()
			...
			if bh.TargetHealthy == False:
				...
				break
			print("Running Block ...")
			hc.SwitchMwAndWait(False)#if molecules prepared in F=1			

			if (i==0):
				hc.UpdateBottomProbeMicrowaveMixerV(0)
				hc.UpdateTopProbeMicrowaveMixerV(0)
			if (i==1):
				hc.UpdateBottomProbeMicrowaveMixerV(0.7)
				hc.UpdateTopProbeMicrowaveMixerV(0)
			if (i==2):
				hc.SwitchMwAndWait(True)#if molecules are prepared in F=1, then true
				hc.UpdateBottomProbeMicrowaveMixerV(0.7)
				hc.UpdateTopProbeMicrowaveMixerV(0.7)
			count=count+1
			sm.AcquireAndWait(2)
