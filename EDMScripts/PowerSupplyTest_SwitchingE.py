...
def Acquire():
	...
	settingsPath = fileSystem.Paths["settingsPath"] + "\\BlockHead\\"
	bc = loadBlockConfig(settingsPath + "powerSupplyTest_SwitchingE.xml")

	x = [1+0.03*i for i in range(0,7)]

	for i in range(7):
		sm.AdjustProfileParameter("out", "externalParameters", str(x[i]), False)
		print(str(hc.ERampUpTime))
		hc.SetEOvershootFactor(x[i])
		for k in range(3):
			bh.StartPattern()
			...
			print("Overshoot Factor is: " + str(x[i]))
			count=count+1

			sm.AcquireStart(1)
			polarity = hc.EFieldPolarity
			time.sleep(15)
			for m in range(3):
				#hc.SwitchEAndWait(not(polarity))
				hc.SwitchE()
				time.sleep(1)
