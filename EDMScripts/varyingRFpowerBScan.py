		...
		for i in range(len(x)):
			...
			for k in range(2):
				...
				if bh.TargetHealthy == False:
					...
					break
				print("Running Block ...")
				hc.SetRF2AttCentre(x[i])
				hc.UpdateRFPowerMonitor()
				sm.AdjustProfileParameter("out", "externalParameters", str(hc.RF2AttCentre), False)
				if (k<1):
					hc.SwitchMwAndWait(True)
				else:
					hc.SwitchMwAndWait(False)
				#print("MW state: " + str(hc.MwSwitchState) + ", Pi flip: " + str(hc.phaseFlip1State))
				print("rf attenuator voltage = " + str(x[i]) + "V, MW state: " + str(hc.MwSwitchState))
				print("Grean synth on amplitude = "+ str(hc.GreenSynthOnAmplitude))
				count=count+1
				sm.AcquireAndWait(2)
