...
.DESCRIPTION
  If you have the .NET SDK and VS Code's C# Dev Kit, prefer the MOTMaster.ScriptTests
  project at the repo root instead of this script - it gives one test per script in VS
  Code's Test Explorer (and `dotnet test` from the repo root works headless too). This
  script is the fallback for a session that only has csc.exe: same check, console output.
  ...
  that throws when Convert.ChangeType'd. This builds ScriptSmokeTest.cs (in this same
  folder) together with ../../../MOTMaster.ScriptTests/ScriptCompileCheck.cs (the actual
  check logic - shared, not duplicated, with the xUnit project) and runs the result,
...
$sharedLogic   = Join-Path $repoRoot "MOTMaster.ScriptTests\ScriptCompileCheck.cs"
if (-not (Test-Path $sharedLogic)) {
    throw "$sharedLogic does not exist - ScriptSmokeTest.cs depends on it (shared check logic, not duplicated)."
}
