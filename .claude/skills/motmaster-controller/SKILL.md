...
| `MOTMaster.ScriptTests/` | xUnit tests, one per script — "will MOTMaster accept this edit" without opening MOTMaster. See Verifying a change below. |
...
`PatternLength`, which a bare compile can't. Two ways to run it, same underlying check
(`MOTMaster.ScriptTests/ScriptCompileCheck.cs`):

- **VS Code Test Explorer (preferred for a human at the keyboard)** — the
  `MOTMaster.ScriptTests` project at the repo root, one `[Theory]` case per top-level
  script. Needs the .NET SDK (C# Dev Kit needed **10** specifically in the version
  installed this session, not just "6+" like its own docs say — this machine now has
  5.0.406, 8.0.424, and 10.0.400 side by side) and the
  `ms-dotnettools.csdevkit`/`ms-dotnettools.csharp` extensions signed in and enabled
  (`dotnet.preferCSharpExtension` must be `false` in `.vscode/settings.json` — `true`
  silently disables Dev Kit's entire UI, including Solution Explorer, with no error
  shown anywhere except a quiet line in the "C# Dev Kit" Output channel).

  **Open only the `MOTMaster.ScriptTests` folder as its own VS Code window** (File → Open
  Folder), not the whole repo/`EDMSuite.sln`. `MOTMaster.ScriptTests.csproj` is
  deliberately **not** added to `EDMSuite.sln` — it was tried and reverted this session:
  Dev Kit's "Build and Run Tests" builds the *entire active solution* before running
  anything, and `EDMSuite.sln` has ~100 mostly-unrelated legacy projects, at least one of
  which (`SirCachealot.csproj`) doesn't build clean under the newer MSBuild bundled with
  the .NET 10 SDK (`MSB3822`, a `System.Resources.Extensions` resx issue, nothing to do
  with MOTMaster). Keeping the test project out of the big solution sidesteps that
  entirely rather than fixing unrelated projects' build errors just to make tests runnable.

  Once opened as its own folder: build once so tests are discovered, then use the beaker
  icon; red/green per script, click through to the failure message. Or headless:
  `dotnet test` from `MOTMaster.ScriptTests/` (or from the repo root, naming the project:
  `dotnet test MOTMaster.ScriptTests`) — works regardless of any VS Code/Dev Kit state.
- **`verify-scripts.ps1` (no dotnet/VS Code needed)** — the original CLI form, useful in
  a plain agent session or CI. Compiles `ScriptSmokeTest.cs` together with the *same*
  `ScriptCompileCheck.cs` the xUnit project uses (not a duplicate) via `csc.exe`, then
  runs it:
  ...
  Takes `-Configuration`/`-ScriptFolder` for a different experiment's scripts.

Both ran this session: **15 of 19 scripts pass, 4 fail** — all four fail for the same
...
- **`*.csproj` is globally gitignored in this repo** (`.gitignore` line 27, alongside
  `*.dll`) — every tracked `.csproj` (`MOTMaster.csproj`, `DAQ.csproj`,
  `MOTMaster.ScriptTests.csproj`, ...) had to be `git add -f`'d in at some point. Adding a
