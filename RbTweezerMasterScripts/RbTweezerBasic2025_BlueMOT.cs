 ...
 * */
{
    public Patterns()
    {
        ...
        Parameters["tScanCount"] = 6831.682;
        ...
        Parameters["tBlueMOTccValue"] = 0.01; // 1.8 for blue MOT
        Parameters["tcMOTccValue"] = 0.48;
        Parameters["tMOTccSwitch"] = 5.5;

        // Shim Coils Default
        Parameters["tShimXccValue"] = 8.8; // 8.8; //8.8; //  0.0
        Parameters["tShimYccValue"] = -2.0; // 3.8;//3.8; // -2.0
        Parameters["tShimZccValue"] = -0.85; // 0.3; //0.3; // -0.85

        // Shim Coils Molasses
        Parameters["tMolassesShimXccValue"] = 10.0; // 8.8 // 10.0 \\ 8.8
        Parameters["tMolassesShimYccValue"] = 3.29; // 3.8 // 3.29 \\ -2.0
        Parameters["tMolassesShimZccValue"] = 0.52; // 0.3 // 0.52 \\ -0.85
        ...
        Parameters["tMOTCool"] = 3.8; //Cool at 2.4 - 2.6   3.8
        ...
        Parameters["tMolCool"] = 1.0;
        Parameters["tMolRep"] = 1.5;
        Parameters["tMolCoolVVA"] = 1.0;
        Parameters["tMolRepVVA"] = 1.0;
        ...
        Parameters["tTweezerBackgroundDelay"] = 200;
    ...
    public override PatternBuilder32 GetDigitalPattern()
    {
        ...
        int BlueMOTStart = RedMolassesStart + (int)Parameters["tRbRedMolassesDuration"];
        int ImgTOFStart = BlueMOTStart + (int)Parameters["tRbBlueMOTDuration"];
        ...
        p.AddEdge("tD1AOMSwitch", RedMolassesStart, false); // Turn on D1 light for sequence
        ...
        // Tweezer
       p.AddEdge("tTweezerSwitch", RedMolassesStart + (int)Parameters["TweezerONDelay"]-10000, true); // Turn OFF tweezer switch at start of red MOT
       p.AddEdge("tTweezerModSwitch", RedMOTStart, true); // Turn OFF tweezer modulation  
       p.AddEdge("tTweezerModSwitch", BlueMOTStart, false); // Turn On tweezer modulation  
       p.AddEdge("tTweezerModSwitch", ImgTOFStart, true); // Turn OFF tweezer modulation
       p.AddEdge("tTweezerSwitch", RedMolassesStart - 10000, false); // Turn ON tweezer switch at start of molasses
       p.AddEdge("tTweezerSwitch", BgStart-500, true); // Turn OFF tweezer switch after imaging to drop atoms
       p.AddEdge("tTweezerSwitch", BgStart, false); // Turn ON tweezer switch after background delay to img background
        ...
        //p.AddEdge("tD1AOMSwitch", ImgStart, false); // Turn on D1 light for sequence
        //p.AddEdge("tD1AOMSwitch", BgTOFStart, true); // Turn off D1 light for sequence
        //p.AddEdge("tD1AOMSwitch", BgStart, false); // Turn on D1 light for sequence
        //p.AddEdge("tD1AOMSwitch", BgStart + (int)Parameters["tTweezerExposure"], true); // Turn off D1 light for sequence


        p.Pulse(ImgStart, 0, 100, "tMOTCamTrig"); // Trigger MOT camera BlueMOTStart ImgStart BlueMOTStart+2000
    ...
    public override AnalogPatternBuilder GetAnalogPattern()
    {
        ...
        p.AddAnalogValue("tMOTcc", RedMolDelayStart, 0.01); // MOT Mag field 
        //p.AddLinearRamp("tMOTcc", RedMolassesStart, 1000, (double)Parameters["tBlueMOTccValue"]);
        //p.AddLinearRamp("tMOTcc", RedMolassesStart, (int)Parameters["tMolassesDelay"], (double)Parameters["tBlueMOTccValue"]); // Ramp down magnetic field 
        ...
        // Imaging 
        p.AddAnalogValue("tRbCoolVCO", ImgTOFStart-10, (double)Parameters["tImgCool"]);
