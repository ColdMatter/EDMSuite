 ...
 * */
{
    public Patterns()
    {
        ...
        Parameters["tMOTCool"] = 4.2; // Max MOT fluo at 4.0
        Parameters["tMOTRep"] = 4.0; // 3.6
        Parameters["tMOTCoolVVA"] = 5.0; // between 0 and 1 V
        Parameters["tMOTRepVVA"] = 5.0;

        // Imaging Cooling and Repump VCO & VVA
        Parameters["tImgCool"] = 4.2; // 3.6 - 4.0, resonance is 4.0
        Parameters["tImgRep"] = 4.0; // 3.0
        Parameters["tImgCoolVVA"] = 5.0;
        Parameters["tImgRepVVA"] = 5.0;

        // Molasses Cooling and Repump VCO & VVA
        Parameters["tMolCool"] = 0.8;
        Parameters["tMolRep"] = 0.8;
        ...
        Parameters["tRbRedMOTDuration"] = 200000;
        Parameters["tMolassesDelay"] = 1;
        Parameters["tRbRedMolassesDuration"] = 1000;
        Parameters["tRbBlueMOTDuration"] = 1;
        Parameters["tRbBlueMolassesDuration"] = 1;
        Parameters["tImgTOF"] = 200;
    ...
    public override PatternBuilder32 GetDigitalPattern()
    {
        ...
        // Tweezer
        p.AddEdge("tTweezerSwitch", RedMOTStart, true); // Turn ON tweezer switch at start of red MOT
        p.AddEdge("tTweezerModSwitch", RedMOTStart, true); // Turn OFF tweezer modulation 
        p.AddEdge("tTweezerSwitch", BgTOFStart, false); // Turn OFF tweezer switch after imaging to drop atoms
        p.AddEdge("tTweezerSwitch", BgStart, true); // Turn ON tweezer switch after background delay to img background
        ...
        p.Pulse(ImgStart, 0, 100, "tHamCamTrig"); // Trigger tweezer camera for image
        p.Pulse(BgStart, 0, 100, "tHamCamTrig"); // Trigger tweezer camera for background
    ...
    public override AnalogPatternBuilder GetAnalogPattern()
    {
        ...
        p.AddAnalogValue("tMOTcc", RedMolDelayStart, 0.0);
