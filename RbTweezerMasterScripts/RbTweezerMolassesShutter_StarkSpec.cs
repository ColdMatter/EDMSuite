 ...
 * */
{
    public Patterns()
    {
        ...
        Parameters["TOF2"] = 2500; // Freefall and expansion
        ...
        Parameters["tMOTCoolSpec"] = 0.0; //
        Parameters["tMOTRepSpec"] = 0.0; // 
        Parameters["tMOTCoolKnock"] = 5.0; //
        Parameters["tMOTRepKnock"] = 5.0; // 
    ...
    public override PatternBuilder32 GetDigitalPattern()
    {
        ...
        // Molasses Lasers
        p.Pulse(rbMolassesStart, 0, (int)Parameters["tRbMolassesDuration"], "tCoolSwitch"); // -200 : pump F=1 -> F=2
        p.Pulse(rbMolassesStart, 0, (int)Parameters["tRbMolassesDuration"]-200, "tRepSwitch"); // -200 : pump F=2 -> F=1

        // Spectroscopy light
        //p.Pulse(TOF2Start+1, 0, (int)Parameters["tSpec"]-2, "tCoolSwitch");
        p.Pulse(TOF2Start+1, 0, (int)Parameters["tSpec"]-2, "tRepSwitch");

        // Knock Out light
        p.Pulse(TOF2Start+(int)Parameters["tSpec"]+1, 0, (int)Parameters["tKnock"]-2, "tCoolSwitch");
        //p.Pulse(TOF2Start+(int)Parameters["tSpec"]+1, 0, (int)Parameters["tKnock"]-2, "tRepSwitch");
    ...
    public override AnalogPatternBuilder GetAnalogPattern()
    {
        ...
        // Spec Laser VCOs
        p.AddAnalogValue("tRbCoolVCO", TOF2Start, (double)Parameters["tMOTCoolSpec"]);// Cooling spec Frequency
        p.AddAnalogValue("tRbRepVCO", TOF2Start, (double)Parameters["tMOTRepSpec"]);// Repump spec Frequency

        // Knock out Laser VCOs
        p.AddAnalogValue("tRbCoolVCO", TOF2Start + (int)Parameters["tSpec"], (double)Parameters["tMOTCoolKnock"]);// Cooling knock out Frequency
        p.AddAnalogValue("tRbRepVCO", TOF2Start + (int)Parameters["tSpec"], (double)Parameters["tMOTRepKnock"]);// Repump knock out Frequency

        // Imaging Laser VCOs
        p.AddAnalogValue("tRbCoolVCO", TOF2Start + (int)Parameters["tKnock"] + (int)Parameters["tSpec"], (double)Parameters["tMOTCoolImg"]);// Img Cooling Frequency
        p.AddAnalogValue("tRbRepVCO", TOF2Start + (int)Parameters["tKnock"] + (int)Parameters["tSpec"], (double)Parameters["tMOTRepImg"]);// Img Repump Frequency
