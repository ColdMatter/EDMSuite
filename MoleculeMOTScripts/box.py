...
BOX_MOTMASTERDATA_PATH = "C:\Users\cafmot\Box\CaF MOT\StateOfExperiment\mot_master_data"
BOX_NOTEBOOK_PATH = "C:\Users\cafmot\Box\CaF MOT\StateOfExperiment\ipython_notebooks"
LOG_PATH = os.path.join("C:\ControlPrograms\EDMSuite\MoleculeMOTScripts", "box.log")

now = datetime.now()
local_motmasterdata_files_list = glob.glob(LOCAL_MOTMASTERDATA_PATH + '/*.zip')
local_notebook_files_list = glob.glob(LOCAL_NOTEBOOK_PATH)

# motmaster backup
if os.path.exists(BOX_MOTMASTERDATA_PATH):
    for local_file in local_motmasterdata_files_list:
        basename = os.path.basename(local_file)
        remote_file = os.path.join(BOX_MOTMASTERDATA_PATH, basename)
...
# notebook backup
if os.path.exists(BOX_NOTEBOOK_PATH):
    for local_file in local_notebook_files_list:
        basename = os.path.basename(local_file)
        backup_file = os.path.join(BOX_NOTEBOOK_PATH, basename)
