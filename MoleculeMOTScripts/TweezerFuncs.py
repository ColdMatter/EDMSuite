...
def gravifitfun(t,t0,M):
    return M*0.5*9.81*(t-t0)**2
...
def GetODDirect(self, clouds, probes, bgs):
    #path="C:\\Users\\cafmot\\Box Sync\\CaF MOT\\MOTData\\MOTMasterData\\CaF19Dec1900\\"
    probes=probes-bgs
    clouds=clouds-bgs
    clouds[clouds<=0]=1.0
...
#        guessedParams = np.array([np.amax(imagearray),np.where(imagearray == np.amax(imagearray))[0][0],np.where(imagearray == np.amax(imagearray))[1][0], 50, 50, 0])
        ...
        params = fitgaussian(imagedata, guessedParams)
        Datafig = TmpFig.imshow(imagedata, cmap='rainbow')
        fig.colorbar(Datafig, ax=TmpFig)
        TmpFig.plot(params[2],params[1], '+')
    ...
    #plot fit along x
    plt.plot(times1**2/(1e-6), (SigX)**2/(1e-6), 'r.' , label = 'widths axially test')
...
#    plt.legend()
#    plt.show()
    print('axial position '+str(np.array(lstXPos)[0]*2*6.5e-6/0.4))
    print('radial position '+str(np.array(lstYPos)[0]*2*6.5e-6/0.4))
    
    fig, axs = plt.subplots(2,1,figsize=(6,6))
    fig.suptitle(imagename +str(seqno), fontsize=14)
    TmpFig = axs[0]
    Xpositions = np.array(lstXPos)*2*6.5e-6/0.4
    TmpFig.plot(times1, Xpositions, '--ro', label = ' max position axially')
    Ypositions = np.array(lstYPos)*2*6.5e-6/0.4
...
#    TmpFig.xticks(np.arange(-47, 383, 20))
    ...
    TmpFig.legend()
    TmpFig = axs[1]
    TmpFig.plot(times1, np.array(lstXwidths)*2*6.5e-6/0.4, '--ro', label = ' width axially')
    TmpFig.plot(times1, np.array(lstYwidths)*2*6.5e-6/0.4, '--bo', label = ' width radially')
#    TmpFig.xticks(np.arange(-47, 383, 20))
...
#        if crop == True:
#            imageData = lstodavg[k][xmin:xmax,ymin:ymax]
#        Datafig=TmpFig.imshow(imageData, cmap='rainbow',vmax=0.24, vmin=0)
    imageData = lstodavg[0] 

    plt.imshow(imageData, cmap='rainbow',vmax=0.24, vmin=0)
    plt.title('test')
    plt.show()
...
#    TmpFig = axs[2]
#    TmpFig.plot(range(1,len(imageData[int(0.5*(ymax-ymin)),:])+1),imageData[int(0.5*(ymax-ymin)),:], 'r.')
def PlotFluoImageNew(path, imagename, seqno,numtrial, numavg,times1, imagecentre, cropsizeX, cropsizeY, crop):
    ...
    fig.suptitle(imagename +str(seqno), fontsize=14)
    for k in range(0,numtrial):
        clouds = plt.imread(path+imagename + str(seqno) + "_" + str(k+1) + ".tif")
        TmpFig = axs[k]
        if crop == True:
...
def GetTemperatureFromFluo(path,imagename, seqno,numtrial, numavg,times1, imagecentre, cropsizeX, cropsizeY, crop,    imgsizey, imgsizex):
    ...
    lstclouds=[];lstprobes=[];lstcloudavg=[];lstods=[];lstodavg=[];totodlst=[];errtotodlst=[];
    for i in range(1,numtrial+1):
        cloudsavg=np.zeros((imgsizex,imgsizey));totod=[];
        for j in range(((i-1)*numavg)+1,i*numavg+1):
            clouds = plt.imread(path+imagename + str(seqno) + "_" + str(j) + ".tif")
    
            lstclouds.append(clouds);
            cloudsavg=cloudsavg+clouds
        cloudsavg=cloudsavg/numavg
        lstcloudavg.append(cloudsavg)
    ...
    for k in range(0,numtrial):
        ...
        if crop == True:
            imagedata = lstcloudavg[k][xmin:xmax,ymin:ymax]
        if crop == False:
            imagedata = lstcloudavg[k]
        
    
        guessedParams = moments(imagedata)+(100,)
    ...
    #converting widths from px to m
    SigX = 8*6.5e-6*(1/0.3)*np.array(lstXwidths)[1:5]
    SigY = 8*6.5e-6*(1/0.3)*np.array(lstYwidths)[1:5]
    times1 = times1[1:5]
    #fit along x
    popt, pcov = curve_fit(linfunc, times1**2, (SigX)**2)
    print('Temperature along axial direction is '+str(int(((87*1.66e-27/(1.38e-23))*popt[0])/(1e-6)))+' muK')
...
def GetPositionsFromFluo(path,imagename, seqno,numtrial, numavg,times1, imagecentre, cropsizeX, cropsizeY, crop,imgsizey,imgsizex):
    ...
    print('axial position '+str(np.array(lstXPos)[0]*8*6.5e-3/0.3))
    print('radial position '+str(np.array(lstYPos)[0]*8*6.5e-3/0.3))
    
    fig, axs = plt.subplots(2,1,figsize=(6,6))
    fig.suptitle(imagename +str(seqno), fontsize=14)
    TmpFig = axs[0]
    Xpositions = np.array(lstXPos)*8*6.5e-6/0.3
    TmpFig.plot(times1, Xpositions, '--ro', label = ' max position axially')
    Ypositions = np.array(lstYPos)*8*6.5e-6/0.3
    TmpFig.plot(times1,Ypositions, '--bo', label = ' max position radially')
    TmpFig.set_ylabel('cloud position in mm')
    TmpFig.legend()
    TmpFig = axs[1]
    TmpFig.plot(times1, np.array(lstXwidths)*8*6.5e-6/0.3, '--ro', label = ' width axially')
    TmpFig.plot(times1, np.array(lstYwidths)*8*6.5e-6/0.3, '--bo', label = ' width radially')
    #    TmpFig.xticks(np.arange(-47, 383, 20))
    TmpFig.set_xlabel('wait time in ms')
    TmpFig.set_ylabel('cloud width in mm')
